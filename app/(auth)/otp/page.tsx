    ...
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-background">
      ...
      <main className="relative z-10 w-full max-w-[480px] px-4 md:px-0">
        ...
        <div className="glass-panel border border-outline-variant/30 rounded-xl p-4 md:p-8 shadow-sm">
          <div className="mb-4">
            <h2 className="text-headline-lg-mobile md:text-headline-lg text-on-surface mb-2">Confirm Identity</h2>
          ...
          <div className="mt-6 flex flex-col gap-3">
            <div className="flex items-start gap-2 bg-surface-container/50 p-3 rounded-lg">
              <span className="material-symbols-outlined text-primary-fixed-dim mt-0.5">info</span>
              <p className="text-body-sm text-on-surface-variant leading-relaxed">
                Security Notice: ES Payment will never ask for your authentication codes over the phone or via email.
              </p>
            </div>
            <div className="flex items-center justify-center gap-6 opacity-60">
              {["iso", "soc2", "secure"].map((cert) => (
                <div key={cert} className="h-8 w-12 bg-surface-container rounded" />
              ))}
            </div>
          </div>
        </div>
